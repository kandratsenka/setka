    ...
    public function postData($data=null){
       $arr=explode(',',$data); 
        ...
        foreach($arr as $one){
            ...
            if($obj){
            $obj_user=MovieUser::where('movie_id',$obj->id)->where('user_id',Auth::user()->id)->first();
            if(!$obj_user){
                $obj_user = new MovieUser;
                $obj_user->movie_id=$obj->id;
                $obj_user->user_id=Auth::user()->id;
                $obj_user->save();
            }
        }}
        ...
        foreach($obj_movies as $movie){
            ...
            if (!in_array($movie->movies->name, $arr1)) {
                MovieUser::where('id',$movie->id)->where('user_id',Auth::user()->id)->delete();
            }
        } 
        }
    }
